...
public enum TTCarouselViewTransformType: Int {
    case crossFading
    case zoomOut
    case depth
    case overlap
...
open class TTCarouselViewTransformer: NSObject {
    
    open internal(set) weak var carouselView: TTCarouselView?
    
    open internal(set) var type: TTCarouselViewTransformType
    
    open var minimumScale: CGFloat = 0.65
    open var minimumAlpha: CGFloat = 0.6
    
    public init(type: TTCarouselViewTransformType) {
        self.type = type
        switch type {
        case .zoomOut:
            minimumScale = 0.85
    ...
    /// 转换属性
    /// attributes - zIndex：Int，frame：CGRect，alpha：CGFloat，transform：CGAffineTransform或transform3D：CATransform3D
    public func applyTransform(to attributes: TTCarouselViewLayoutAttributes) {
        guard let carouselView = carouselView else { return }
        let position = attributes.position
        let scrollDirection = carouselView.scrollDirection
    ...
    open func proposedInteritemSpacing() -> CGFloat {
        guard let carouselView = carouselView else { return 0 }
        let scrollDirection = carouselView.scrollDirection
        switch type {
        case .overlap:
            guard scrollDirection == .horizontal else { return 0 }
            return carouselView.itemSize.width * -minimumScale * 0.6
        case .linear:
            guard scrollDirection == .horizontal else { return 0 }
            return carouselView.itemSize.width * -minimumScale * 0.2
        case .coverFlow:
            guard scrollDirection == .horizontal else { return 0 }
            return -carouselView.itemSize.width * sin(.pi * 0.25 * 0.25 * 3)
        case .ferrisWheel, .invertedFerrisWheel:
            guard scrollDirection == .horizontal else { return 0 }
            return -carouselView.itemSize.width * 0.15
        case .cubic: return 0
        default:
            break
        }
