...
class TTFMHomeVipViewModel: NSObject {
    ...
    /// 更新回调
    var updateDataHandler: ((String?, RequestErrorCode) -> Void)?
}

extension TTFMHomeVipViewModel {
    /// 刷新数据
    func refreshData() {
        TTFMNetworkManager.shared.request(target: TTFMHomeVipAPI.vipList) { (json, message, code) in
            ...
            if let categoryBtnList = JSONDeserializer<TTFMCategoryButtonModel>.deserializeModelArrayFrom(json: json?["categoryContents"]["list"][0]["list"].description) as? [TTFMCategoryButtonModel] {
                self.categoryBtnList = categoryBtnList
            }
            
            self.updateDataHandler?(json?["message"].description, code)
        }
    }
}
